...
NOVEL_DISPLAY_NAMES = {name: name for name in ("Avocado", "Blueberry", "Cherry", "Kiwi", "Mango", "Rockmelon")}
KNOWN_DISPLAY_NAMES = {name: name for name in ("Apple", "Banana", "Orange", "Strawberry", "Pineapple")}
...
def render(row: dict, assignments: dict[str, dict], truth: dict, output: Path) -> None:
    ...
    for detection in row["known_detections"]:
        ...
        text = f"{KNOWN_DISPLAY_NAMES.get(detection['class_name'], detection['class_name'])} {detection['confidence']:.2f}"
    ...
    for proposal in row["unknown_proposals"]:
        ...
        text = f"Unknown fruit C{cluster}" if cluster is not None else "Unknown fruit"
        if candidate:
            text += f" / {NOVEL_DISPLAY_NAMES.get(candidate, candidate)}?"
    ...
    footer_draw.text((22, image.height + 12), "Box-level open-world detection - yellow dashed boxes are unknown candidates", fill="#0F172A", font=font(25, True))
    footer_draw.text((22, image.height + 51), f"Independent-test reference class: {NOVEL_DISPLAY_NAMES.get(category, category)}", fill="#475569", font=font(20))
...
def main() -> None:
    ...
    selected = []
    for category in NOVEL_DISPLAY_NAMES:
