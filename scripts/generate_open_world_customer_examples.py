...
CLASS_DISPLAY_NAMES = {name: name for name in ("Avocado", "Blueberry", "Cherry", "Kiwi", "Mango", "Rockmelon")}
...
def make_card(record: dict, truth: str, mapped: str | None, destination: Path) -> None:
    ...
    draw.text((34, 20), "Open-category discovery (image level)", fill="white", font=font(36, bold=True))
    draw.text((830, 28), "Independent holdout sample", fill="white", font=font(25))
    ...
    semantic_text = CLASS_DISPLAY_NAMES[truth] if is_semantic_match else "Name requires human confirmation"
    draw.text((35, footer_y + 25), "Decision: novel-category candidate", fill="#0F172A", font=font(31, bold=True))
    draw.text((520, footer_y + 25), f"Discovered cluster: {record['cluster_id']}", fill="#334155", font=font(29))
    draw.text((35, footer_y + 78), f"Post-hoc cluster mapping: {semantic_text}", fill=color, font=font(29, bold=True))
    draw.text((760, footer_y + 78), f"Novelty: {novelty:.2f}", fill="#0F172A", font=font(29, bold=True))
    draw.text(
        (35, footer_y + 128),
        f"Test reference class: {CLASS_DISPLAY_NAMES[truth]} - a higher novelty score means less similarity to the five known fruits",
        fill="#475569",
        font=font(23),
    )
    draw.text(
        (35, footer_y + 170),
        "Note: this result shows image-level unknown-category discovery and clustering, not box-level open-world detection.",
        fill="#64748B",
        font=font(20),
    )
...
def main() -> None:
    ...
    written: list[Path] = []
    for truth in CLASS_DISPLAY_NAMES:
        candidates = sorted(
            grouped[truth],
            key=lambda item: (
    ...
    summary = {
        ...
        "classes": list(CLASS_DISPLAY_NAMES),
