...
def render(item: dict, output: Path) -> None:
    ...
    for detection in item["detections"]:
        ...
        text = f"{DISPLAY_NAMES.get(detection['name'], detection['name'])} {detection['confidence']:.2f}"
    ...
    foot.text((20, image.height + 10), "Incremental open-category detection - solid coloured boxes are model predictions", fill="#0F172A", font=font(25, True))
    foot.text((20, image.height + 49), f"Protected-test reference class: {DISPLAY_NAMES.get(category, category)}", fill="#475569", font=font(20))
