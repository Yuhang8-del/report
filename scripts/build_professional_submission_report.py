...
def build_inventory(out: Path, report_path: Path, pdf_path: Path | None, worktree: Path, runtime: Path):
    items=[
        ("Original requirement","Course report guidelines",Path(r"E:\bishe\fruit\MSc - Advanced Project Guidelines (2025-26).docx"),"Reviewed"),
        ("Original requirement","Project execution plan",Path(r"E:\bishe\fruit\execution_plan.docx"),"Reviewed"),
        ("Original requirement","Requirements presentation",Path(r"E:\bishe\fruit\requirements_presentation.pptx"),"Reviewed; excluded from this delivery"),
        ("Data","data.yaml",out/"data/data.yaml","Prepared"),("Data","class_names.yaml",out/"data/class_names.yaml","Prepared"),("Data","data_manifest.csv",out/"data/data_manifest.csv","Prepared"),
        ("Data","10/20/40/100 budget manifests",out/"data/splits/budget_100.txt","Prepared; four files are stored together"),("Data","unlabeled_train.txt",out/"data/splits/unlabeled_train.txt","Prepared"),
        ("Data","Pseudo-label and filtering record",runtime/"artifacts_v17/pseudo/v3_teacher_r3_seed42/filter/audit.jsonl","Completed; large raw evidence is not duplicated"),
        ("Code","Training, pseudo-label, evaluation and visualisation scripts",worktree/"src/fruit_ssod","Completed"),("Code","README",worktree/"README.md","Completed"),("Code","requirements.txt/lock",worktree/"requirements-lock.txt","Completed"),
        ("Model","Teacher best.pt",runtime/"artifacts_v17/runs/supervised-v3-domain-balanced-yolov8m-1024-seed42-r3/weights/best.pt","Completed"),("Model","Student best.pt",runtime/"artifacts_v17/runs/ssod-v3-teacher-r3-student-seed42/weights/best.pt","Completed"),
        ("Result","baseline_results.xlsx",out/"results/baseline_results.xlsx","Prepared"),("Result","ssod_results.xlsx",out/"results/ssod_results.xlsx","Prepared"),("Result","trust_filter_ablation.xlsx",out/"results/trust_filter_ablation.xlsx","Prepared; reports only available comparative evidence"),("Result","deployment_benchmark.xlsx",out/"results/deployment_benchmark.xlsx","Prepared; three-image demonstration, not a formal throughput benchmark"),
        ("Software","English PySide6 demonstration application",worktree/"scripts/start_gui.ps1","Completed; image, folder, video and external camera"),("Report","Final Report Word",report_path,"Generated"),("Report","Final Report PDF",pdf_path or out/"final_report.pdf","Generated"),
        ("Pending test","Formal AP-small and selective-tiling comparison",out/"results/deployment_benchmark.xlsx","Insufficient current evidence; no value fabricated"),("Pending extension","Box-level open-world detection and new-class registration",runtime/"artifacts_v17/open_world/post_student_ssod-v3-teacher-r3-student-seed42/discovery_results.json","Image-level discovery completed; extension interface reserved"),
    ]
    tex_path=out/"final_report.tex"
    if tex_path.exists(): items.insert(-2,("Report","Final Report LaTeX source",tex_path,"Generated; reproducible with XeLaTeX"))
    csv_path=out/"required_deliverables_inventory.csv"
    with csv_path.open("w",encoding="utf-8-sig",newline="") as f:
        w=csv.writer(f); w.writerow(["Category","Required file or deliverable","Actual path","Status","Exists"])
        for cat,name,path,status in items: w.writerow([cat,name,str(path),status,"Yes" if path.exists() else "No"])
    md=["# Original Requirements and Required Deliverables","","This inventory follows the course report guidelines, project execution plan and requirements presentation. Early exploratory low-score runs are omitted from the formal report but retained in the runtime evidence archive.","","| Category | Required file or deliverable | Status | Actual path |","|---|---|---|---|"]
    for cat,name,path,status in items: md.append(f"| {cat} | {name} | {status} | `{path}` |")
    md.extend(["","## Outside This Delivery","","- Defence slides, English script and recording instructions were removed after customer confirmation.","- Full box-level open-world detection is not claimed; the current delivery provides image-level additional-category discovery with reserved extension interfaces.","","## Demonstration Inputs","","- The English PySide6 GUI supports local images, folders, videos and real-time external-camera inference.","","## Author Actions Before Submission","","- Complete the name, student ID and supervisor fields on the cover.","- Follow the course-platform rules if anonymous submission is required.","- Run a dedicated benchmark if formal acceptance requires AP-small or batch throughput; do not substitute latency from three demonstration images."])
    md_path=out/"ORIGINAL_REQUIREMENTS_AND_DELIVERABLES.md"; md_path.write_text("\n".join(md)+"\n",encoding="utf-8")
